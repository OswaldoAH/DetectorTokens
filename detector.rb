			 			...
			 			if cad.length>@palabraReservada[i].length
			 				if !((cadena1.eql?"while") || (cadena1.eql?"if")||(cadena1.eql?"do"))
								if cadena1.eql?(@palabraReservada[i])
									"Error en la linea #{index+1} "
									"Linea completa con el error #{cad}"
									return true
								else
									cont=0
									if !cad.eql?"false"
		...
		Shoes.app :title=>"Detector de Tokens",
			...
			stack :width=>"50%" do
				...
				flow :width=>"100%" do
					...
					@imgSelect = image("seleccionar.png", :top=>140, :left =>145, :width=>100, :height=>100)
					@imgSelect.click do
					...
					@imgPlay = image("play.png", :top=>140, :left => 295, :width => 100, :height=>100)
					@imgPlay.click do
